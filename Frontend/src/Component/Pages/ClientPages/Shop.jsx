...
const Shop = () => {
  ...
  // State for pagination, products per page, and view mode
  ...
  const [productsPerPage, setProductsPerPage] = useState(maxProductsPerPage); // Dynamic products per page
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center py-3 px-4 md:px-6 lg:px-25 bg-[#F5F5F5] gap-3 md:gap-4 lg:gap-0">
        ...
        <div className="flex flex-wrap items-center justify-center lg:justify-end gap-2 md:gap-4 lg:gap-5">
          <div className="space-x-1 md:space-x-2 lg:space-x-4">
            ...
            <select
              id="showing"
              className="px-1 lg:px-3 py-1 text-black bg-white focus:outline-none focus:ring-2 focus:ring-[#b68e4c] h-8 lg:h-[55px] w-[40px] lg:w-[55px] text-xs lg:text-base text-center appearance-none"
      ...
      <div className="w-full max-w-[1200px] mx-auto">
        ...
        >
          {currentProducts.map((product) => (
            <div
              key={product.id}
              className={`group relative bg-white shadow-md rounded-xl overflow-hidden transition-all duration-300 hover:shadow-lg ${
                viewMode === "grid"
                  ? "w-full max-w-[285px] h-[380px] sm:h-[400px] md:h-[420px] lg:h-[446px] mx-auto"
                  : "w-full h-auto flex flex-col sm:flex-row items-center sm:items-start gap-4 p-4"
              }`}
            >
              {/* Product Image and Badge */}
              <div
                className={
                  viewMode === "grid" ? "relative" : "relative w-full sm:w-1/3"
                }
              >
                <img
                  src={product.image}
                  ...
                  className={
                    viewMode === "grid"
                      ? "w-full h-[220px] sm:h-[250px] md:h-[280px] lg:h-[301px] object-cover rounded-t-xl"
                      : "w-full h-[200px] object-cover rounded-xl sm:rounded-l-xl sm:rounded-r-none"
                  }
                />
                {(product.discount || product.badge) && (
                  <div
                    className={`absolute top-2 sm:top-3 right-2 sm:right-3 flex flex-row items-end gap-1 sm:gap-2`}
                  >
                    {product.discount && (
                      <span
                        className={`flex items-center justify-center bg-[#E97171] text-white font-medium rounded-full ${
                          viewMode === "grid"
                            ? "h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 text-xs sm:text-sm md:text-base"
                            : "h-8 w-8 text-xs"
                        }`}
                      >
                        {product.discount}
                    ...
                    {product.badge && (
                      <span
                        className={`flex items-center justify-center bg-[#2EC1AC] text-white font-medium rounded-full ${
                          viewMode === "grid"
                            ? "h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 text-xs sm:text-sm md:text-base"
                            : "h-8 w-8 text-xs"
                        }`}
              ...
              <div
                className={
                  viewMode === "grid"
                    ? "p-3 sm:p-4 h-[160px] sm:h-[150px] md:h-[140px] lg:h-[145px] w-full py-2 sm:py-3 md:py-4"
                    : "flex-1 p-3 sm:p-4 w-full sm:w-2/3"
                }
              >
                <h3
                  className={
                    viewMode === "grid"
                      ? "text-lg sm:text-xl md:text-2xl text-[#3A3A3A] leading-[120%] font-semibold"
                      : "text-lg sm:text-xl text-[#3A3A3A] leading-[120%] font-semibold"
                  }
                >
                  {product.name}
                </h3>
                ...
                <div
                  className={
                    viewMode === "grid"
                      ...
                      : "flex flex-col sm:flex-row sm:items-center sm:justify-between mt-2"
                  }
                >
                  <span
                    className={
                      viewMode === "grid"
                        ? "text-base sm:text-lg md:text-xl text-[#3A3A3A] leading-[150%] font-semibold"
                        : "text-base sm:text-lg text-[#3A3A3A] leading-[150%] font-semibold"
                    }
                  >
                    {product.price}
                  </span>
                  {product.oldPrice && (
                    <span
                      className={
                        viewMode === "grid"
                          ? "text-[#B0B0B0] text-xs sm:text-sm md:text-base line-through leading-[150%]"
                          : "text-[#B0B0B0] text-xs sm:text-sm line-through leading-[150%]"
                      }
                    >
                      {product.oldPrice}
                    </span>
                ...
                {viewMode === "list" && (
                  <div className="mt-3 flex flex-col sm:flex-row gap-2">
                    <button
                      className="bg-[#B88E2F] text-white font-semibold py-2 px-4 rounded-md hover:bg-[#a07d2a] transition-colors"
                      aria-label={`Add ${product.name} to cart`}
                    >
                      Add to cart
                    </button>
                    <div className="flex gap-2 sm:gap-4 text-[#3A3A3A] font-semibold">
                      <span
                        className="flex items-center gap-1 hover:cursor-pointer hover:text-[#B88E2F] transition-colors text-xs sm:text-sm"
                        aria-label="Share product"
                      >
                        <IoShareSocialSharp className="text-xs sm:text-sm" />{" "}
                        Share
                      </span>
                      <span
                        className="flex items-center gap-1 hover:cursor-pointer hover:text-[#B88E2F] transition-colors text-xs sm:text-sm"
                        aria-label="Compare product"
                      >
                        <GoArrowSwitch className="text-xs sm:text-sm" /> Compare
                      </span>
                      <span
                        className="flex items-center gap-1 hover:cursor-pointer hover:text-[#B88E2F] transition-colors text-xs sm:text-sm"
                        aria-label="Like product"
                      >
                        <FaRegHeart className="text-xs sm:text-sm" /> Like
                      </span>
                    </div>
                  </div>
                )}
